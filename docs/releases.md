...
| -------------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REACT_ICONS_SKIP_METADATA_DIFF_CHECK` | `build-verify` test (react-icons) | When set to `true`, skips the `git diff metadata.json` check. Required in the publish workflow because `metadata.json` is regenerated by the build but committed later in the pipeline. Must **not** be set in PR validation so stale metadata is caught. |
