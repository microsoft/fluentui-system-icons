...
# 2. Ensure you're on the correct branch and have the latest updates:
#    - Use `git fetch` or `git pull` to ensure your local repository is up-to-date.
#    - Use `git checkout <branch>` to switch to the intended branch if necessary.
# 3. Run the script: `./scripts/diff.sh`
# 4. The script will prompt you to enter two commit IDs.
...
# 5. The script will create a tarball of the changed files, extract them, and organize them into specified directories.
# 6. It will then move PDF and SVG files to separate directories and open these directories.
# 7. Finally, it will list files with more than 3 lines and perform a check in the all_files_svg directory.
...
# If commit_id_to is empty, set it to the current commit ID
if [ -z "$commit_id_to" ]; then
    commit_id_to=$(git rev-parse HEAD)
fi
