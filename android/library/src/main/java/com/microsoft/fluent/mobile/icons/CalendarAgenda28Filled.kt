package com.microsoft.fluent.mobile.icons

import androidx.compose.material.icons.materialPath
import androidx.compose.ui.graphics.vector.ImageVector
import com.microsoft.fluent.mobile.FluentIcons
import com.microsoft.fluent.mobile.fluentIcon

public val FluentIcons.Filled.CalendarAgenda28: ImageVector
  get() {
    if (_calendarAgenda28 != null) {
      return _calendarAgenda28!!
    }
    _calendarAgenda28 = fluentIcon(name = "Filled.CalendarAgenda28", 28f) {
      materialPath {
          moveTo(25.0F, 21.75F)
          curveToRelative(0.0F, 1.795F, -1.455F, 3.25F, -3.25F, 3.25F)
          horizontalLineTo(6.25F)
          curveTo(4.455F, 25.0F, 3.0F, 23.545F, 3.0F, 21.75F)
          verticalLineTo(6.25F)
          curveTo(3.0F, 4.455F, 4.455F, 3.0F, 6.25F, 3.0F)
          horizontalLineToRelative(15.5F)
          curveTo(23.545F, 3.0F, 25.0F, 4.455F, 25.0F, 6.25F)
          verticalLineToRelative(15.5F)
          close()
          moveTo(19.75F, 14.0F)
          curveToRelative(0.0F, -0.38F, -0.282F, -0.694F, -0.648F, -0.743F)
          lineTo(19.0F, 13.25F)
          horizontalLineTo(9.0F)
          lineToRelative(-0.102F, 0.007F)
          curveTo(8.532F, 13.307F, 8.25F, 13.62F, 8.25F, 14.0F)
          reflectiveCurveToRelative(0.282F, 0.694F, 0.648F, 0.743F)
          lineTo(9.0F, 14.75F)
          horizontalLineToRelative(10.0F)
          lineToRelative(0.102F, -0.007F)
          curveToRelative(0.366F, -0.05F, 0.648F, -0.363F, 0.648F, -0.743F)
          close()
          moveToRelative(0.0F, 5.25F)
          curveToRelative(0.0F, -0.38F, -0.282F, -0.694F, -0.648F, -0.743F)
          lineTo(19.0F, 18.5F)
          horizontalLineTo(9.0F)
          lineToRelative(-0.102F, 0.007F)
          curveTo(8.532F, 18.557F, 8.25F, 18.87F, 8.25F, 19.25F)
          reflectiveCurveToRelative(0.282F, 0.694F, 0.648F, 0.743F)
          lineTo(9.0F, 20.0F)
          horizontalLineToRelative(10.0F)
          lineToRelative(0.102F, -0.007F)
          curveToRelative(0.366F, -0.05F, 0.648F, -0.363F, 0.648F, -0.743F)
          close()
          moveToRelative(0.0F, -10.5F)
          curveToRelative(0.0F, -0.38F, -0.282F, -0.694F, -0.648F, -0.743F)
          lineTo(19.0F, 8.0F)
          horizontalLineTo(9.0F)
          lineTo(8.898F, 8.007F)
          curveTo(8.532F, 8.057F, 8.25F, 8.37F, 8.25F, 8.75F)
          reflectiveCurveToRelative(0.282F, 0.694F, 0.648F, 0.743F)
          lineTo(9.0F, 9.5F)
          horizontalLineToRelative(10.0F)
          lineToRelative(0.102F, -0.007F)
          curveToRelative(0.366F, -0.05F, 0.648F, -0.363F, 0.648F, -0.743F)
          close()        
      }
    }
    return _calendarAgenda28!!
  }

private var _calendarAgenda28: ImageVector? = null
