package com.microsoft.fluent.mobile.icons

import androidx.compose.material.icons.materialPath
import androidx.compose.ui.graphics.vector.ImageVector
import com.microsoft.fluent.mobile.FluentIcons
import com.microsoft.fluent.mobile.fluentIcon

public val FluentIcons.Filled.HourglassThreeQuarter24: ImageVector
  get() {
    if (_hourglassThreeQuarter24 != null) {
      return _hourglassThreeQuarter24!!
    }
    _hourglassThreeQuarter24 = fluentIcon(name = "Filled.HourglassThreeQuarter24", 24f) {
      materialPath {
          moveTo(6.97F, 7.0F)
          curveToRelative(0.24F, 0.397F, 0.565F, 0.738F, 0.95F, 1.0F)
          lineTo(10.0F, 9.48F)
          curveToRelative(0.42F, 0.278F, 0.77F, 0.65F, 1.02F, 1.089F)
          curveToRelative(0.524F, 0.916F, 0.524F, 1.946F, 0.0F, 2.862F)
          curveToRelative(-0.25F, 0.438F, -0.6F, 0.811F, -1.02F, 1.089F)
          lineTo(7.92F, 16.0F)
          curveToRelative(-0.433F, 0.294F, -0.789F, 0.689F, -1.036F, 1.15F)
          curveToRelative(-0.247F, 0.462F, -0.379F, 0.976F, -0.384F, 1.5F)
          verticalLineToRelative(0.6F)
          curveToRelative(0.0F, 0.066F, 0.026F, 0.13F, 0.073F, 0.177F)
          curveTo(6.62F, 19.474F, 6.683F, 19.5F, 6.75F, 19.5F)
          horizontalLineToRelative(10.5F)
          curveToRelative(0.066F, 0.0F, 0.13F, -0.026F, 0.177F, -0.073F)
          curveToRelative(0.047F, -0.047F, 0.073F, -0.11F, 0.073F, -0.177F)
          verticalLineToRelative(-0.6F)
          curveToRelative(-0.005F, -0.524F, -0.137F, -1.038F, -0.384F, -1.5F)
          curveToRelative(-0.247F, -0.461F, -0.603F, -0.856F, -1.036F, -1.15F)
          lineTo(14.0F, 14.52F)
          curveToRelative(-0.42F, -0.278F, -0.77F, -0.65F, -1.02F, -1.089F)
          curveToRelative(-0.524F, -0.916F, -0.524F, -1.946F, 0.0F, -2.862F)
          curveToRelative(0.25F, -0.438F, 0.6F, -0.811F, 1.02F, -1.089F)
          lineTo(16.08F, 8.0F)
          curveToRelative(0.385F, -0.262F, 0.71F, -0.603F, 0.95F, -1.0F)
          horizontalLineTo(6.97F)
          close()        
      }
    }
    return _hourglassThreeQuarter24!!
  }

private var _hourglassThreeQuarter24: ImageVector? = null
