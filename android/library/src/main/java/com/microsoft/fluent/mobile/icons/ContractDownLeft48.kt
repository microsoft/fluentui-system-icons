package com.microsoft.fluent.mobile.icons

import androidx.compose.material.icons.materialPath
import androidx.compose.ui.graphics.vector.ImageVector
import com.microsoft.fluent.mobile.FluentIcons
import com.microsoft.fluent.mobile.fluentIcon

public val FluentIcons.Regular.ContractDownLeft48: ImageVector
  get() {
    if (_contractDownLeft48 != null) {
      return _contractDownLeft48!!
    }
    _contractDownLeft48 = fluentIcon(name = "Regular.ContractDownLeft48", 48f) {
      materialPath {
          moveTo(41.634F, 8.134F)
          curveToRelative(0.488F, -0.488F, 0.488F, -1.28F, 0.0F, -1.768F)
          reflectiveCurveToRelative(-1.28F, -0.488F, -1.768F, 0.0F)
          lineTo(29.5F, 16.732F)
          verticalLineTo(7.25F)
          curveTo(29.5F, 6.56F, 28.94F, 6.0F, 28.25F, 6.0F)
          reflectiveCurveTo(27.0F, 6.56F, 27.0F, 7.25F)
          verticalLineToRelative(12.5F)
          curveToRelative(0.0F, 0.69F, 0.56F, 1.25F, 1.25F, 1.25F)
          horizontalLineToRelative(12.5F)
          curveToRelative(0.69F, 0.0F, 1.25F, -0.56F, 1.25F, -1.25F)
          reflectiveCurveToRelative(-0.56F, -1.25F, -1.25F, -1.25F)
          horizontalLineToRelative(-9.482F)
          lineTo(41.634F, 8.134F)
          close()
          moveTo(8.5F, 12.25F)
          curveToRelative(0.0F, -2.07F, 1.679F, -3.75F, 3.75F, -3.75F)
          horizontalLineToRelative(7.885F)
          curveToRelative(0.69F, 0.0F, 1.25F, -0.56F, 1.25F, -1.25F)
          reflectiveCurveTo(20.825F, 6.0F, 20.135F, 6.0F)
          horizontalLineTo(12.25F)
          curveTo(8.798F, 6.0F, 6.0F, 8.798F, 6.0F, 12.25F)
          verticalLineToRelative(23.5F)
          curveTo(6.0F, 39.202F, 8.798F, 42.0F, 12.25F, 42.0F)
          horizontalLineToRelative(23.5F)
          curveToRelative(3.452F, 0.0F, 6.25F, -2.798F, 6.25F, -6.25F)
          verticalLineToRelative(-7.884F)
          curveToRelative(0.0F, -0.69F, -0.56F, -1.25F, -1.25F, -1.25F)
          reflectiveCurveToRelative(-1.25F, 0.56F, -1.25F, 1.25F)
          verticalLineToRelative(7.884F)
          curveToRelative(0.0F, 2.071F, -1.679F, 3.75F, -3.75F, 3.75F)
          horizontalLineTo(24.0F)
          verticalLineTo(29.25F)
          curveToRelative(0.0F, -2.9F, -2.35F, -5.25F, -5.25F, -5.25F)
          horizontalLineTo(8.5F)
          verticalLineTo(12.25F)
          close()
          moveToRelative(0.0F, 14.25F)
          horizontalLineToRelative(10.25F)
          curveToRelative(1.519F, 0.0F, 2.75F, 1.231F, 2.75F, 2.75F)
          verticalLineTo(39.5F)
          horizontalLineToRelative(-9.25F)
          curveToRelative(-2.071F, 0.0F, -3.75F, -1.679F, -3.75F, -3.75F)
          verticalLineTo(26.5F)
          close()        
      }
    }
    return _contractDownLeft48!!
  }

private var _contractDownLeft48: ImageVector? = null
