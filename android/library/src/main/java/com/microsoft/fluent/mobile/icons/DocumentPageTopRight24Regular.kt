package com.microsoft.fluent.mobile.icons

import androidx.compose.material.icons.materialPath
import androidx.compose.ui.graphics.vector.ImageVector
import com.microsoft.fluent.mobile.FluentIcons
import com.microsoft.fluent.mobile.fluentIcon

public val FluentIcons.Regular.DocumentPageTopRight24: ImageVector
  get() {
    if (_documentPageTopRight24 != null) {
      return _documentPageTopRight24!!
    }
    _documentPageTopRight24 = fluentIcon(name = "Regular.DocumentPageTopRight24", 24f) {
      materialPath {
          moveTo(17.744F, 1.996F)
          curveToRelative(1.19F, 0.0F, 2.166F, 0.925F, 2.245F, 2.096F)
          lineToRelative(0.005F, 0.154F)
          verticalLineToRelative(15.498F)
          curveToRelative(0.0F, 1.191F, -0.925F, 2.166F, -2.096F, 2.245F)
          lineToRelative(-0.154F, 0.005F)
          horizontalLineToRelative(-11.5F)
          curveToRelative(-1.19F, 0.0F, -2.165F, -0.925F, -2.245F, -2.096F)
          lineToRelative(-0.005F, -0.154F)
          verticalLineTo(4.246F)
          curveToRelative(0.0F, -1.19F, 0.925F, -2.166F, 2.096F, -2.245F)
          lineToRelative(0.154F, -0.005F)
          horizontalLineToRelative(11.5F)
          close()
          moveToRelative(0.0F, 1.5F)
          horizontalLineToRelative(-11.5F)
          curveToRelative(-0.38F, 0.0F, -0.693F, 0.282F, -0.743F, 0.648F)
          lineTo(5.494F, 4.246F)
          verticalLineToRelative(15.498F)
          curveToRelative(0.0F, 0.38F, 0.282F, 0.694F, 0.648F, 0.743F)
          lineToRelative(0.102F, 0.007F)
          horizontalLineToRelative(11.5F)
          curveToRelative(0.38F, 0.0F, 0.694F, -0.282F, 0.743F, -0.648F)
          lineToRelative(0.007F, -0.102F)
          verticalLineTo(4.246F)
          curveToRelative(0.0F, -0.38F, -0.282F, -0.693F, -0.648F, -0.743F)
          lineToRelative(-0.102F, -0.007F)
          close()
          moveToRelative(-2.726F, 8.523F)
          curveToRelative(-0.409F, -0.065F, -0.688F, -0.449F, -0.623F, -0.858F)
          lineToRelative(0.104F, -0.66F)
          horizontalLineToRelative(-1.48F)
          lineToRelative(-0.143F, 0.895F)
          curveToRelative(-0.064F, 0.409F, -0.449F, 0.688F, -0.858F, 0.623F)
          curveToRelative(-0.409F, -0.065F, -0.688F, -0.449F, -0.623F, -0.858F)
          lineToRelative(0.104F, -0.66F)
          horizontalLineToRelative(-0.75F)
          curveToRelative(-0.414F, 0.0F, -0.75F, -0.337F, -0.75F, -0.75F)
          curveTo(10.0F, 9.335F, 10.335F, 9.0F, 10.75F, 9.0F)
          horizontalLineToRelative(0.988F)
          lineToRelative(0.237F, -1.5F)
          horizontalLineTo(11.25F)
          curveToRelative(-0.414F, 0.0F, -0.75F, -0.337F, -0.75F, -0.75F)
          curveTo(10.5F, 6.335F, 10.835F, 6.0F, 11.25F, 6.0F)
          horizontalLineToRelative(0.963F)
          lineToRelative(0.2F, -1.26F)
          curveToRelative(0.064F, -0.41F, 0.448F, -0.688F, 0.857F, -0.623F)
          curveToRelative(0.41F, 0.064F, 0.689F, 0.448F, 0.624F, 0.858F)
          lineTo(13.73F, 6.0F)
          horizontalLineToRelative(1.48F)
          lineToRelative(0.2F, -1.259F)
          curveToRelative(0.065F, -0.41F, 0.45F, -0.688F, 0.858F, -0.623F)
          curveToRelative(0.41F, 0.064F, 0.689F, 0.448F, 0.624F, 0.858F)
          lineTo(16.73F, 6.0F)
          horizontalLineToRelative(0.518F)
          curveTo(17.664F, 6.0F, 18.0F, 6.336F, 18.0F, 6.75F)
          curveToRelative(0.0F, 0.415F, -0.335F, 0.75F, -0.75F, 0.75F)
          horizontalLineToRelative(-0.756F)
          lineTo(16.256F, 9.0F)
          horizontalLineToRelative(0.493F)
          curveToRelative(0.415F, 0.0F, 0.75F, 0.336F, 0.75F, 0.75F)
          reflectiveCurveToRelative(-0.335F, 0.75F, -0.75F, 0.75F)
          horizontalLineToRelative(-0.73F)
          lineToRelative(-0.143F, 0.896F)
          curveToRelative(-0.064F, 0.409F, -0.449F, 0.688F, -0.858F, 0.623F)
          close()
          moveTo(13.493F, 7.5F)
          lineTo(13.256F, 9.0F)
          horizontalLineToRelative(1.48F)
          lineToRelative(0.238F, -1.5F)
          horizontalLineToRelative(-1.48F)
          close()        
      }
    }
    return _documentPageTopRight24!!
  }

private var _documentPageTopRight24: ImageVector? = null
