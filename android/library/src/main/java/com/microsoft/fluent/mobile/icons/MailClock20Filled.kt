package com.microsoft.fluent.mobile.icons

import androidx.compose.material.icons.materialPath
import androidx.compose.ui.graphics.vector.ImageVector
import com.microsoft.fluent.mobile.FluentIcons
import com.microsoft.fluent.mobile.fluentIcon

public val FluentIcons.Filled.MailClock20: ImageVector
  get() {
    if (_mailClock20 != null) {
      return _mailClock20!!
    }
    _mailClock20 = fluentIcon(name = "Filled.MailClock20", 20f) {
      materialPath {
          moveTo(19.0F, 5.5F)
          curveToRelative(0.0F, 2.485F, -2.015F, 4.5F, -4.5F, 4.5F)
          reflectiveCurveTo(10.0F, 7.985F, 10.0F, 5.5F)
          reflectiveCurveTo(12.015F, 1.0F, 14.5F, 1.0F)
          reflectiveCurveTo(19.0F, 3.015F, 19.0F, 5.5F)
          close()
          moveTo(14.5F, 3.0F)
          curveTo(14.224F, 3.0F, 14.0F, 3.224F, 14.0F, 3.5F)
          verticalLineToRelative(2.0F)
          curveTo(14.0F, 5.776F, 14.224F, 6.0F, 14.5F, 6.0F)
          horizontalLineTo(16.0F)
          curveToRelative(0.276F, 0.0F, 0.5F, -0.224F, 0.5F, -0.5F)
          reflectiveCurveTo(16.276F, 5.0F, 16.0F, 5.0F)
          horizontalLineToRelative(-1.0F)
          verticalLineTo(3.5F)
          curveTo(15.0F, 3.224F, 14.776F, 3.0F, 14.5F, 3.0F)
          close()
          moveToRelative(0.0F, 8.0F)
          curveToRelative(1.33F, 0.0F, 2.55F, -0.472F, 3.5F, -1.257F)
          verticalLineTo(14.5F)
          curveToRelative(0.0F, 1.38F, -1.12F, 2.5F, -2.5F, 2.5F)
          horizontalLineToRelative(-11.0F)
          curveTo(3.12F, 17.0F, 2.0F, 15.88F, 2.0F, 14.5F)
          verticalLineTo(7.373F)
          lineToRelative(7.747F, 4.558F)
          curveToRelative(0.156F, 0.092F, 0.35F, 0.092F, 0.507F, 0.0F)
          lineToRelative(2.23F, -1.312F)
          curveTo(13.108F, 10.865F, 13.788F, 11.0F, 14.5F, 11.0F)
          close()
          moveToRelative(-10.0F, -7.0F)
          horizontalLineToRelative(4.707F)
          curveTo(9.072F, 4.477F, 9.0F, 4.98F, 9.0F, 5.5F)
          curveToRelative(0.0F, 1.906F, 0.97F, 3.585F, 2.442F, 4.572F)
          lineTo(10.0F, 10.92F)
          lineTo(2.015F, 6.223F)
          curveTo(2.153F, 4.973F, 3.213F, 4.0F, 4.5F, 4.0F)
          close()        
      }
    }
    return _mailClock20!!
  }

private var _mailClock20: ImageVector? = null
