...
describe('Build Verification', () => {
  ...
  describe('Promoted default output', () => {
    ...
    it('keeps the deprecated ./headless subpaths resolvable as re-export shims', () => {
      // `finalizeCjs` renames the CommonJS output, so the shim extension is format-dependent.
      for (const [libDir, ext] of [
        ['lib', 'js'],
        ['lib-cjs', 'cjs'],
      ]) {
        for (const stem of ['headless/index', 'headless/utils', 'headless/fonts/index']) {
          const file = `${stem}.${ext}`;
    ...
    it('no longer emits CSS-in-JS style artefacts', () => {
      ...
      for (const libDir of ['lib', 'lib-cjs']) {
        ...
        for (const entry of fs.readdirSync(root, { recursive: true })) {
          ...
          // `.cjs` as well as `.js`: `finalizeCjs` renames the CommonJS output, so a stray
          // `lib-cjs/**/*.styles.js` would reach the published package as `*.styles.cjs`.
          if (/[.]styles([.]raw)?[.]c?js$/.test(rel)) {
