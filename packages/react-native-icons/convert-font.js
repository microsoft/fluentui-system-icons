...
async function processFolder(srcPath, codepointMapDestFolder, resizable) {
  var files = await glob(
    resizable ? 'FluentSystemIcons-Resizable.json' : 'FluentSystemIcons-{Filled,Regular,Light}.json',
...
async function generateCodepointMapForWebpackPlugin(destPath, iconEntries, resizable) {
  const finalCodepointMap = Object.fromEntries(
    Object.entries(iconEntries).map(([name, codepoint]) => [getReactIconNameFromGlyphName(name, resizable), codepoint]),
  );
...
function generateReactIconEntries(iconEntries, resizable) {
  ...
  for (const [iconName, codepoint] of Object.entries(iconEntries)) {
    ...
    let iconStyle = /filled$/i.test(iconName)
      ? 0 /* Filled */
      : /regular$/i.test(iconName)
        ? 1 /* Regular */
        : 3; /* Light */

    var jsCode = `export const ${destFilename} = /*#__PURE__*/createFluentFontIcon(${JSON.stringify(
      destFilename,
    )}, ${JSON.stringify(String.fromCodePoint(codepoint))}, ${resizable ? 2 /* Resizable */ : iconStyle}${
