...
// @ts-check
/**
 * Runs the font subsetting plugin test builds against webpack and/or rspack.
 *
 * Each bundler compiles the same five fixture entries; the assertion plugin in `make-configs.js`
 * throws when a font asset exceeds its subset size threshold.
 *
...
async function main() {
  ...
  for (const bundler of selected) {
    console.log(`\n=== Running font subsetting tests with ${bundler} ===\n`);
...
function runBundler(bundler) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    compilerFactory(configs, (err, stats) => {
      ...
      console.log(stats.toString({ colors: true, preset: 'errors-warnings', assets: true }));
