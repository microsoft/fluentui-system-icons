...
`@fluentui/react-icons` is ESM-first: it is marked `"type": "module"` and every entry point declares both an `import` (native ESM, `lib/`) and a `require` (CommonJS, `lib-cjs/` as `.cjs` + `.d.cts`) condition.

This is packaging only — no API surface changed. Bundlers see no difference, `require()` keeps working, and bare Node `import` now resolves without a bundler. It does **not** require you to switch to `"moduleResolution": "node16"`; `"bundler"` remains the recommendation above.
...
👉 **[Build-Time Transform setup (Webpack) →](?path=/docs/icons-build-transforms--docs)**
