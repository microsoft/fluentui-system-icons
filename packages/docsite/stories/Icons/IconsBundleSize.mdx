...
<h1 className="sbdocs-title">Bundle Size: Icon Rendering Approaches</h1>

This page compares the JavaScript bundle footprint of the different icon rendering approaches — **inline SVG**, **font**, and **SVG sprites** — across the **standard (Griffel)** and **[Headless](?path=/docs/icons-headless-api--docs)** APIs, with and without CSS extraction.
...
>
> _Headless variants use vanilla CSS (not Griffel). With CSS extraction (`MiniCssExtractPlugin`) the CSS moves out of JS entirely, yielding smaller JS bundles. Without CSS extraction (`style-loader`), webpack's css-loader/style-loader runtime is added to the JS bundle — see the analysis below._
...
> ⚠️ **Preview:** SVG sprites are a [preview feature](?path=/docs/icons-rendering-approaches--docs) of the standard (Griffel) API, and a **headless** sprite variant is not published yet. The `Headless` rows below are experimental measurements shown for reference only.
...
**With CSS extraction (recommended for production):** Headless is the clear winner across all icon types — **−70% for fonts**, **−56% for sprites** _(experimental — not published yet)_, **−14% for SVG inline** (minified). This is the ideal setup where vanilla CSS is fully extracted to `.css` files with zero JS runtime overhead.
...
> See the [Headless API](?path=/docs/icons-headless-api--docs) page for how to adopt the headless variants, and the [Rendering Approaches](?path=/docs/icons-rendering-approaches--docs) page for choosing between inline SVG, fonts, and sprites.
